...
#     "postgresql://postgres:12345@localhost:5432/projectdatabase"
# )
...
class Invitations(db.Model):
    ...
    def __repr__(self):
        return "<Invitations (id:%r, sender_id:%r, recipient_id:%r)>" % (
            self.id,
            self.sender_id,
            self.recipient_id,
        )
...
def profile():
    ...
    try:
        ...
        house_list = [
            db.session.query(House).filter(House.id == invitation.house_id).first()
            for invitation in invitation_list
            if invitation.house_id != None
        ]
    except:
        house_list = []
...
def add_house():
    ...
    if request.method == "POST":
        ...
        house = House(
            user_id=g.users.id, name=name, description=description, address=address
        )
        ...
        invitation = Invitations(
            sender_id=g.users.id,
            recipient_id=g.users.id,
            house_id=house.id,
            status="accepted",
        )
...
def notifications():
    ...
    return render_template(
        "notifications.html", house_list=house_list, invitation_list=invitation_list
    )
...
def accept_invitation(invitation_id):
    invitation = (
        db.session.query(Invitations).filter(Invitations.id == invitation_id).first()
    )
...
def tracker(house_id):
    ...
    debtors = {
        k: v for k, v in sorted(debtors.items(), key=lambda item: item[1], reverse=True)
    }
...
def user_list(house_id):
    ...
    invitation_list = (
        db.session.query(Invitations).filter(Invitations.house_id == house_id).all()
    )
    ...
    invitation_list_ids_pending = [
        invitation.recipient_id for invitation in invitation_list_pending
    ]
    ...
    if invitation_list_ids_pending:
        ...
        user_list_pending = (
            db.session.query(Users)
            .filter(
                and_(Users.id != g.users.id, Users.id.in_(invitation_list_ids_pending))
            )
            .all()
        )
    else:
...
def invite_user(house_id, user_id):
    ...
    invitation = Invitations(
        sender_id=g.users.id, recipient_id=user_id, house_id=house_id, status="pending"
    )
...
def edit_item(house_id, item_id):
    ...
    if request.method == "POST":
        ...
        if (
            int(request.form.get("quantity")) != item_quantity
            or int(request.form.get("quantity")) != item.quantity
        ):
    ...
    return render_template(
        "edit_item.html", item=item, item_quantity=item_quantity, house_id=house_id
    )
        ...
        for borrower in participants:
            if owner.recipient_id != borrower.recipient_id:
                if owner.recipient_id == g.users.id:
                    debtors[borrower.recipient] = sum(
                        [
                            taker.item.price / taker.item.quantity
                            for taker in takers
                            if taker.user_id == borrower.recipient_id
                            and taker.item.author_id == g.users.id
                        ]
                    )
                if borrower.recipient_id == g.users.id:
                    debts[owner.recipient] = sum(
                        [
                            taker.item.price / taker.item.quantity
                            for taker in takers
                            if taker.user_id == borrower.recipient_id
                            and taker.item.author_id == owner.recipient_id
                        ]
                    )

    return render_template(
        "debts.html", debtors=debtors, debts=debts, house_id=house_id
    )
